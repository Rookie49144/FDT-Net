...
def dce_eviloss(p, alpha, c, global_step, annealing_step):
    ...
    U = c / S
    weight = torch.exp(-U)
    weight = weight.reshape(2, 1, 512, 512)
    alpha1 = alpha1 * weight
    prediction = torch.softmax(alpha1, 1)
    L_dice = softmax_dice(alpha1, p)
    E = alpha - 1

    label = p.view(p.size(0), p.size(1), -1)  # [N, C, HW]
    label = label.transpose(1, 2)  # [N, HW, C]
    label = label.contiguous().view(-1, label.size(2))
    # digama loss
    d = torch.digamma(S) - torch.digamma(alpha)
    e = label * d
    L_ace = torch.sum(e, dim=1, keepdim=True)
    annealing_coef = min(1, global_step / annealing_step)
    alp = E * (1 - label) + 1
    L_KL = annealing_coef * KL(alp, c)

    return (0.3 * L_dice +  L_ace + L_KL), prediction
